...
public class BoardPanel extends JLayeredPane {
    ...
    private Point getPoint(int width, int height, int position, int plus) {
        ...
        int rand = plus * 10;
        if (position == 0) {
            return new Point(12 * width / 13, 8 * height / 9 + rand);
        }
        if (position == 13) {
            return new Point(width / 17, 8 * height / 9 + rand);
        }
        if (position == 21) {
            return new Point(width / 17, height / 11 + rand);
        }
        if (position == 34) {
            return new Point(12 * width / 13, height / 11 + rand);
        }
        if (position < 13) {
            int twidth = width / 4;
