...
public class Board {
	...
	public void step() {
		...
		if (currentPlayer.getState().canStep(currentPlayer, dice))
			stepOn(newPosition);
	...
	public void stepOn(int newPosition) {
		...
		Field currentField = fields[currentPlayer.getPosition()];

		if (canBuyProperties(currentPlayer, currentField)) {
			queueImmediateAction(new ShowMessageGameAction("You can buy properties that you dont already own!"));
		}

		queueImmediateAction(fields[newPosition].getAction());
	}

	private boolean canBuyProperties(Player player, Field field) {
		return !getAvailableProperties(player, field).isEmpty();
	}

	private Set<Property> getAvailableProperties(Player player, Field field) {
		return field.getProperties().stream().filter(propForField -> !player.getProperties().contains(propForField))
